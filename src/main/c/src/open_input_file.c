...
int open_input_file(const char *filename, input_ctx *in, JNIEnv *env) {
    ...
    if ((ret = avformat_open_input(&in->fmt_ctx, filename, NULL, NULL)) < 0) {
        fmt_msg_throw(env, "Could not open input file '%s' (error '%s')\n",
    ...
    if ((ret = avformat_find_stream_info(in->fmt_ctx, NULL)) < 0) {
        fmt_msg_throw(env, "Could not get stream info (error '%s')\n",
                        av_err2str(ret));
        goto cleanup;
    }
    ...
    in->stream_idx = av_find_best_stream(in->fmt_ctx, AVMEDIA_TYPE_AUDIO,
                                         -1, -1, &input_codec, 0);
    if (in->stream_idx < 0) {
        fmt_msg_throw(env, "Could not find stream (error '%s')\n",
    ...
    if (!(input_codec = avcodec_find_decoder(stream->codecpar->codec_id))) {
        fmt_msg_throw(env, "No suitable decoder found for '%s'\n",
    ...
    if (!(in->codec_ctx = avcodec_alloc_context3(input_codec))) {
        fmt_msg_throw(env, "Failed to allocate decoding context\n");
        ret = AVERROR(ENOMEM);
        goto cleanup;
    }
    ...
    if ((ret = avcodec_open2(in->codec_ctx, input_codec, NULL))) {
        fmt_msg_throw(env, "Could not open input codec (error '%s')\n",
                        av_err2str(ret));
        goto cleanup;
    }
